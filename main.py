...
async def run_csv_validation():
    ...
    try:
        # Видалено імпорт utils.csv_test, бо файл видалено
        logger.warning("CSV validation skipped - module removed")
        return True
...
async def main():
    ...
    # Видалено створення imb_analyzer, vol_analyzer, signal_generator - файли видалені
    executor = TradeExecutor(storage, api_manager)
    
    # Choose orchestrator based on mode
    if settings.technical_analysis.enable_ta_mode:
        # Передаю None замість видалених об'єктів
        orchestrator = TradingOrchestratorTA(storage, None, None, None, executor)
