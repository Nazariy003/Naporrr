...
class DataCollector:
    ...
    async def start(self):
        ...
        # Завантажуємо історичні трейди для TA
        for symbol in self.pairs:
            await self._load_historical_trades(symbol)
            await asyncio.sleep(0.5)  # Не перевантажувати API
    ...
    async def _load_historical_trades(self, symbol: str, limit: int = 1000):
        """Завантажити історичні трейди для TA"""
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(url, params=params) as response:
                    if response.status != 200:
                        logger.warning(f"[HISTORICAL_TRADES] Failed {symbol}: {response.status}")
                        return
                    ...
                    if data.get('retCode') == 0 and data.get('result', {}).get('list'):
                        ...
                        logger.info(f"[HISTORICAL_TRADES] Loaded {len(trades)} trades for {symbol}")
                        ...
                        for trade in trades:
                            ...
                            ts = int(trade['time']) / 1000  # ms to seconds
                            is_aggressive = trade.get('isBlockTrade', False)
                            
                            self.storage.add_trade(symbol, price, size, side, is_aggressive)
                    else:
                        logger.warning(f"[HISTORICAL_TRADES] No data for {symbol}")
        except Exception as e:
            logger.warning(f"[HISTORICAL_TRADES] Error loading {symbol}: {e}")
    ...
    async def _handle_orderbook(self, data: dict):
        ...
        try:
            ...
            # Перевірка на тип даних - якщо це OrderBookSnapshot об'єкт
            if hasattr(ob_data, 'bids') and hasattr(ob_data, 'asks'):
                # Обробка як об'єкта Bybit
                ...
                # Виклик оновлення свічок з orderbook
                await self._update_candle_from_orderbook(symbol, ob_data)
    ...
    async def _update_candle_from_orderbook(self, symbol: str, orderbook):
        """Оновлення свічок з orderbook даних"""
        try:
            # Перевірка типу orderbook
            ...
            if not isinstance(bids, list) or not isinstance(asks, list):
                logger.error(f"Orderbook bids/asks are not lists for {symbol}")
                return
            
            # Тут додати логіку оновлення свічок (наприклад, VWAP або обсяг)
            # Наприклад, якщо є клас свічок в storage
            # self.storage.update_candle_from_orderbook(symbol, bids, asks)
            
            # Тимчасово - тільки логування
            logger.debug(f"Updated candle for {symbol} from orderbook (bids: {len(bids)}, asks: {len(asks)})")
